...
function gaugeBar(pct, width, stops, dimColor, mode, animMode, now, shimmerEnabled, fillChar = "▓", emptyChar = "░") {
  ...
  for (let i = 0; i < width; i++) {
    if (i < filled) {
      ...
      out += paint(rgb, fillChar, mode);
    } else {
      out += paint(dimColor, emptyChar, mode);
    }
  }
  return out;
}
...
function maybeMarquee(text, cfg, k, now, max = 18) {
  if (text.length > max && k.animMode === "active" && cfg.animation.enabled && cfg.animation.marquee) {
    return A.marquee(text, max, now);
  }
  return text;
}
...
export const SEGMENTS = {
  ...
  model: (ctx, cfg, k) => {
    const name = k.P(k.theme.accent2, ctx.model.toUpperCase(), true);
    const effort = ctx.effort ? k.P(k.theme.dim, " · ", false) + k.P(k.theme.warn, ctx.effort.toUpperCase(), false) : "";
    const think = ctx.thinking ? k.P(k.theme.accent, " ⚡", false) : "";
    const fast = ctx.fastMode ? k.P(k.theme.ok, " ⚡", false) : "";
    return k.P(k.theme.accent2, "⟨", false) + name + effort + think + fast + k.P(k.theme.accent2, "⟩", false);
  },
  dir: (ctx, cfg, k) =>
    ctx.dir ? k.P(k.theme.accent2, "▸ ", false) + k.P(k.theme.ok, maybeMarquee(ctx.dir, cfg, k, ctx.now), true) : null,
  git: (ctx, cfg, k) =>
    ctx.branch ? k.P(k.theme.ink, "⎇ ", false) + k.P(k.theme.ink, maybeMarquee(ctx.branch, cfg, k, ctx.now), false) : null,
  context: (ctx, cfg, k) => {
    if (ctx.contextPct == null) return null;
    const stops = [k.theme.ok, k.theme.warn, k.theme.crit];
    const bar = gaugeBar(ctx.contextPct, 10, stops, k.theme.dim, k.mode, k.animMode, ctx.now, cfg.animation.shimmer, "⣿", "⣿");
    const col = gradient(stops, Math.min(1, ctx.contextPct / 100));
    return k.P(k.theme.ink, "CTX", false) + " " + bar + " " + k.P(col, `${ctx.contextPct}%`, true);
  },
  fiveHour: (ctx, cfg, k) => limitSeg("Session", ctx.fiveHour, ctx, cfg, k),
  weekly: (ctx, cfg, k) => limitSeg("Week", ctx.weekly, ctx, cfg, k),
  ...
  lines: (ctx, cfg, k) =>
    ctx.linesAdded || ctx.linesRemoved
      ...
      : null,
  duration: (ctx, cfg, k) => (ctx.durationMs == null ? null : k.P(k.theme.accent2, "⌁ ", false) + k.P(k.theme.dim, fmtDuration(ctx.durationMs), false)),
  clock: (ctx, cfg, k) => {
